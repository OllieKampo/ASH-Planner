...
function make_directories {
    ...
    $path = ".\experiments\results\aggregate\$($parent)"
    if (!(test-path $path)) {
        mkdir $path
    } else {
    ...
    foreach ($child in $children) {
        $path = ".\experiments\results\aggregate\$($parent)\$($child)"
        if (!(test-path $path)) {
            mkdir $path
        } else {
            Get-ChildItem -Path $path -Include *.* -File -Recurse | ForEach-Object { $_.Delete() }
...
Start-Sleep -Seconds 1
make_directories -parent ach_type_search_mode -children ach, search, both, yield, standard, minbound
...
# Each Search Mode
python .\ProcessResults.py .\results\initial\offline\sequential_achievement\ -out .\results\aggregate\ach_type_search_mode\yield\yield -combine all -filter planning_mode=offline search_mode=yield achievement_type=seqa -breakf problem -plots time -excel False -p False -show False
python .\ProcessResults.py .\results\initial\offline\sequential_achievement\ -out .\results\aggregate\ach_type_search_mode\standard\standard -combine all -filter planning_mode=offline search_mode=standard achievement_type=seqa -breakf problem -plots time -excel False -p False -show False
python .\ProcessResults.py .\results\initial\offline\sequential_achievement\ -out .\results\aggregate\ach_type_search_mode\minbound\minbound -combine all -filter planning_mode=offline search_mode=minbound achievement_type=seqa -breakf problem -plots time -excel False -p False -show False
...
python .\ProcessResults.py .\results\initial\online\ .\results\main\improved_initial\preemptive_achievement\optimise\ -out .\results\aggregate\online_bounds\preach_best_bounds\preach -combine all -diff preach_type -same problem -filter search_mode=minbound online_bounds="(2~ 2)" -allow_none all -breakf preach_type -breaks problem -plots grades quality time -p False -show False
python .\ProcessResults.py .\results\initial\online\ .\results\main\improved_initial\preemptive_achievement\optimise\ -out .\results\aggregate\online_bounds\preach_all_bounds\preach -combine all -diff preach_type -same problem -filter search_mode=minbound -allow_none all -breakf preach_type -breaks online_bounds -plots grades quality time -p False -show False
...
# Different Bound Values for PS3 for each online method
...
# Difference between PS3 and PS3TS averaged over bounds
